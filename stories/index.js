...
class TableController extends React.Component {
  ...
  fetch = (page = 1) => {
    fetch(
      'https://api.themoviedb.org/3/movie/top_rated?api_key=' + this.state.api_key + '&page=' + page
    )
      .then(res => res.json())
      .then(json => {
        this.setState({
  ...
  getContextMenu(row, index, key) {
    ...
    return [
      {
        title: 'Edit row',
        onClickHandler: action('Edit row action')
      },
  ...
  getContextHeaderMenu(cell, index, key) {
    return [{ title: 'Header Action', onClickHandler: action('header action') }];
  }
